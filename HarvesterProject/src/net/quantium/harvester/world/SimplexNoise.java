...
public class SimplexNoise {
	...
	public SimplexNoise(int octaves, double frequency, double persistence){
	...
	public double octavedNoise(int x, int y, int w, int h){
		...
		double amplitude = Math.pow(persistence, octaves);
        for(int i = 0; i < octaves; i++){
        	result += SimplexOctave.noise(x / frequency, y / frequency) * amplitude;
	...
	public static class SimplexOctave {  // Simplex noise in 2D, 3D and 4D
		  private static final int SWAPS = 400;  

		  private static Grad grad[] = {new Grad(1,1),new Grad(-1,1),new Grad(1,-1),new Grad(-1,-1),
		  ...
		  public static void update(int seed) {
		  ...
		  static{
			  update(new Random().nextInt(Integer.MAX_VALUE));
		  }
		  ...
		  // is this method are really faster??
